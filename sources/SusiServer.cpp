...
Susi::SusiServer::SusiServer(short port, std::string keyFile,
                             std::string certificateFile)
    : Susi::FramingServer<LineFramer, Susi::SSLTCPServer>{port, keyFile,
                                                          certificateFile} {}
...
void Susi::SusiServer::onConnect(int id) {
  std::cout << "got new client "
            << Susi::SSLTCPServer::getPeerCertificateHash(id) << std::endl;
  Susi::FramingServer<LineFramer, Susi::SSLTCPServer>::onConnect(id);
  BSON::Value sessionNewEvent =
      BSON::Object{{"topic", "core::session::new"}, {"payload", id}};
  publish(sessionNewEvent, 0);
}

void Susi::SusiServer::onClose(int id) {
  std::cout << "lost client " << Susi::SSLTCPServer::getPeerCertificateHash(id)
            << std::endl;
  ...
  for (auto &kv : consumers) {
    kv.second.erase(
        std::remove_if(kv.second.begin(), kv.second.end(), [id](int currentId) {
          return currentId == id;
        }), kv.second.end());
  }
  ...
  for (auto &kv : publishProcesses) {
    ...
    if (process->nextProcessor - 1 < process->processors.size() &&
        process->processors[process->nextProcessor - 1] == id) {
      orphanedProcesses.push_back(process);
    }
    // remove all id occurences from the process
    for (size_t i = process->nextProcessor; i < process->processors.size();
         i++) {
  ...
  BSON::Value sessionLostEvent =
      BSON::Object{{"topic", "core::session::lost"}, {"payload", id}};
...
void Susi::SusiServer::onFrame(int id, std::string &frame) {
  ...
  if (!validateFrame(doc)) {
    BSON::Value res = BSON::Object{
        {"type", "error"},
        {"data", BSON::Object{{"reason", "your request is invalid"}}}};
    send(id, res);
  } else {
...
bool Susi::SusiServer::validateFrame(BSON::Value &doc) {
  if (!doc.isObject() || !doc["type"].isString() || !doc["data"].isObject() ||
...
void Susi::SusiServer::send(int id, BSON::Value &doc) {
  ...
  Susi::FramingServer<LineFramer, Susi::SSLTCPServer>::send(id, frame.c_str(),
                                                            frame.size());
}

void Susi::SusiServer::registerConsumer(std::string &topic, int id) {
  if (!contains(consumers[topic], id)) {
    std::cout << "register consumer " + topic + " for "
              << Susi::SSLTCPServer::getPeerCertificateHash(id) << std::endl;
...
void Susi::SusiServer::registerProcessor(std::string &topic, int id) {
  if (!contains(processors[topic], id)) {
    std::cout << "register processor " + topic + " for "
              << Susi::SSLTCPServer::getPeerCertificateHash(id) << std::endl;
...
void Susi::SusiServer::unregisterConsumer(std::string &topic, int id) {
  for (size_t i = 0; i < consumers[topic].size(); i++) {
    if (id == consumers[topic][i]) {
      std::cout << "unregister consumer " + topic + " for "
                << Susi::SSLTCPServer::getPeerCertificateHash(id) << std::endl;
...
void Susi::SusiServer::unregisterProcessor(std::string &topic, int id) {
  for (size_t i = 0; i < processors[topic].size(); i++) {
    if (id == processors[topic][i]) {
      std::cout << "unregister processor " + topic + " for "
                << Susi::SSLTCPServer::getPeerCertificateHash(id) << std::endl;
...
void Susi::SusiServer::publish(BSON::Value &event, int publisher) {
  std::cout << "publish event " + event["topic"].getString() + " for "
            << Susi::SSLTCPServer::getPeerCertificateHash(publisher)
            << std::endl;
  ...
  if (timeout) {
    ...
    process->timeout.async_wait(
        [this, process](const boost::system::error_code &error) {
          if (!error) {
            process->lastState["headers"].push_back(
                BSON::Object{{"Error", "Timeout"}});
...
void Susi::SusiServer::ack(BSON::Value &event, int acker) {
  if (acker != 0) {
    std::cout << "got ack for event " + event["topic"].getString() + " from "
              << Susi::SSLTCPServer::getPeerCertificateHash(acker) << std::endl;
  }
  ...
  if (process->nextProcessor >= process->processors.size()) {
    ...
    if (!checkForNoConsumerHeader(event)) {
      BSON::Value consumerEvent =
          BSON::Object{{"type", "consumerEvent"}, {"data", event}};
      for (auto &id : process->consumers) {
        std::cout << "send consumer event " + event["topic"].getString() +
                         " to "
                  << Susi::SSLTCPServer::getPeerCertificateHash(id)
                  << std::endl;
        send(id, consumerEvent);
      }
    }
    if (process->publisher != 0 && !checkForNoAckHeader(event)) {
      BSON::Value publisherAck = BSON::Object{{"type", "ack"}, {"data", event}};
      std::cout << "send ack for event " + event["topic"].getString() +
                       " to publisher "
                << Susi::SSLTCPServer::getPeerCertificateHash(
                       process->publisher) << std::endl;
      send(process->publisher, publisherAck);
    }
    publishProcesses.erase(id);
  } else {
    if (!checkForNoProcessorHeader(event)) {
      std::cout << "forward event " + event["topic"].getString() + " to "
                << Susi::SSLTCPServer::getPeerCertificateHash(
                       process->processors[process->nextProcessor])
                << std::endl;
      BSON::Value processorEvent =
          BSON::Object{{"type", "processorEvent"}, {"data", event}};
...
void Susi::SusiServer::dismiss(BSON::Value &event, int acker) {
  if (acker != 0) {
    std::cout << "got dismiss for event " + event["topic"].getString() +
                     " from "
              << Susi::SSLTCPServer::getPeerCertificateHash(acker) << std::endl;
  }
  std::string &id = event["id"];
  ...
  if (!checkForNoConsumerHeader(event)) {
    BSON::Value consumerEvent =
        BSON::Object{{"type", "consumerEvent"}, {"data", event}};
    for (auto &id : process->consumers) {
      std::cout << "send consumer event " + event["topic"].getString() + " to "
                << Susi::SSLTCPServer::getPeerCertificateHash(id) << std::endl;
      send(id, consumerEvent);
    }
  }
  if (process->publisher != 0 && !checkForNoAckHeader(event)) {
    BSON::Value publisherAck =
        BSON::Object{{"type", "dismiss"}, {"data", event}};
    std::cout << "send dismiss for event " + event["topic"].getString() +
                     " to publisher "
              << Susi::SSLTCPServer::getPeerCertificateHash(process->publisher)
              << std::endl;
    send(process->publisher, publisherAck);
  }
  publishProcesses.erase(id);
...
bool Susi::SusiServer::checkForHeader(BSON::Value &event,
...
int64_t Susi::SusiServer::checkForTimeoutHeader(BSON::Value &event) {
  if (event["headers"].isArray()) {
    for (auto &header : event["headers"].getArray()) {
      if (header["Event-Timeout"].isString() &&
          header["Event-Timeout"].isString()) {
