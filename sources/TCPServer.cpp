...
Susi::TCPServer::TCPServer(short port)
    : acceptor{io_service, boost::asio::ip::tcp::endpoint(
                               boost::asio::ip::tcp::v6(), port)} {
  do_accept();
  runloop = std::move(std::thread{[this]() { io_service.run(); }});
}
...
std::string Susi::TCPServer::getPeerCertificateHash(int id) { return ""; }

std::string Susi::TCPServer::getPeerCertificate(int id) { return ""; }
...
void Susi::TCPServer::Session::start() { do_read(); }
